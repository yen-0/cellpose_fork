...
def _pair_candidates_for_track(track_idx: int, anchor: InstanceNode, current_nodes: List[InstanceNode],
                               link_iou: float, candidate_indices: Optional[List[int]] = None):
    candidates = []
    idx_iter = candidate_indices if candidate_indices is not None else range(len(current_nodes))
    for ci in idx_iter:
...
def build_association_tracks(slice_masks, link_iou=0.1, link_dist=30.0, size_tolerance=0.6,
    ...
    for z in z_iter:
        ...
        prefilter_map = None
        if gpu_prefilter and len(prev_tracks) > 0 and len(current_nodes) > 0:
            prefilter_map = _gpu_prefilter_candidates(prev_tracks, current_nodes, link_dist, size_tolerance)

        def _candidate_indices_for_track(track_index: int):
            if prefilter_map is None:
                return None
            return prefilter_map.get(track_index, [])
            ...
            with ThreadPoolExecutor(max_workers=worker_count) as ex:
                futures = [
                    ex.submit(
                        ...
                        current_nodes,
                        link_iou,
                        _candidate_indices_for_track(ti),
                    )
