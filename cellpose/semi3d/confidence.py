...
def track_confidence(track, image_stack, source_masks=None, image_max=None, gray_stack=None):
    ...
    intensity_support = float(np.clip(np.mean(intensity_vals) / (image_max + 1e-6), 0.0, 1.0)) if intensity_vals else 0.5
